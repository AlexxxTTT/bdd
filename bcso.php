        ...
        switch ($_POST['action']) {
            case 'add_agent':
                ...
                if (isset($_FILES['photo_portrait']) && $_FILES['photo_portrait']['error'] == 0) {
                    $photo_portrait = 'uploads/bcso_portraits/' . time() . '_' . $_FILES['photo_portrait']['name'];
                ...
                if (isset($_FILES['photo_identite']) && $_FILES['photo_identite']['error'] == 0) {
                    $photo_identite = 'uploads/bcso_identites/' . time() . '_' . $_FILES['photo_identite']['name'];
                    move_uploaded_file($_FILES['photo_identite']['tmp_name'], $photo_identite);
                }
                
                $stmt = $pdo->prepare("INSERT INTO agents_bcso (nom, prenom, telephone, rang, matricule, unite, grade_unite, notes, photo_portrait, photo_identite) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$nom, $prenom, $telephone, $rang, $matricule, $unite, $grade_unite, $notes, $photo_portrait, $photo_identite]);
                
                $message = "Agent BCSO ajouté avec succès !";
                break;
            ...
            case 'edit_agent':
                ...
                $stmt = $pdo->prepare("SELECT photo_portrait, photo_identite FROM agents_bcso WHERE id = ?");
                $stmt->execute([$id]);
                ...
                $stmt = $pdo->prepare("UPDATE agents_bcso SET nom = ?, prenom = ?, telephone = ?, rang = ?, matricule = ?, unite = ?, grade_unite = ?, notes = ?, photo_portrait = ?, photo_identite = ? WHERE id = ?");
                $stmt->execute([$nom, $prenom, $telephone, $rang, $matricule, $unite, $grade_unite, $notes, $photo_portrait, $photo_identite, $id]);
                
                $message = "Agent BCSO modifié avec succès !";
                break;
            ...
            case 'delete_agent':
                ...
                $stmt = $pdo->prepare("DELETE FROM agents_bcso WHERE id = ?");
                $stmt->execute([$id]);
                
                $message = "Agent BCSO supprimé avec succès !";
                break;
        }
    }
}

// Récupérer tous les agents BCSO
$stmt = $pdo->query("SELECT * FROM agents_bcso ORDER BY nom, prenom");
$agents = $stmt->fetchAll();

// Récupérer un agent spécifique pour modification
$agent_edit = null;
if (isset($_GET['edit'])) {
    $stmt = $pdo->prepare("SELECT * FROM agents_bcso WHERE id = ?");
    $stmt->execute([$_GET['edit']]);
    $agent_edit = $stmt->fetch();
}

// Créer les dossiers s'ils n'existent pas
if (!file_exists('uploads/bcso_portraits')) {
    mkdir('uploads/bcso_portraits', 0777, true);
}
if (!file_exists('uploads/bcso_identites')) {
    mkdir('uploads/bcso_identites', 0777, true);
}

// Listes prédéfinies pour BCSO
$rangs = [
    'Detention Deputy', 'Deputy Sheriff I', 'Deputy Sheriff II', 'Deputy Sheriff III', 
    'Detective I', 'Detective II', 'Detective III', 'Sergeant I', 'Sergeant II', 
    'Lieutenant I', 'Lieutenant II', 'Captain I', 'Captain II', 'Commander', 
    'Assistant Sheriff', 'Undersheriff', 'Sheriff'
];

$unites = [
    'Patrol', 'Traffic', 'K9', 'SWAT', 'Detective', 'Narcotics', 'Gang Unit', 
    'Air Support', 'Marine Unit', 'Motorcycle', 'Detention', 'Court Services',
    'Civil Process', 'Training', 'Administration', 'Emergency Services'
];
...
<head>
    ...
    <title>Gestion des agents BCSO - FiveM Database</title>
        ...
        <nav>
            <div class="nav-container">
                ...
                <ul>
                    ...
                    <li><a href="bcso.php" class="active">BCSO</a></li>
    ...
    <main>
        <div class="container">
            ...
            <div class="form-section">
                ...
                <form method="POST" enctype="multipart/form-data" class="agent-form">
                    ...
                    <div class="form-actions">
                        ...
                        <?php if ($agent_edit): ?>
                            <a href="bcso.php" class="btn btn-secondary">Annuler</a>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
            ...
            <div class="agents-section">
                <h3>Liste des agents BCSO (<?php echo count($agents); ?>)</h3>
                
                <?php if (empty($agents)): ?>
                    <p class="no-data">Aucun agent BCSO enregistré.</p>
                <?php else: ?>
                        ...
                        <?php foreach ($agents as $agent): ?>
                            <div class="agent-card">
                                ...
                                <div class="agent-actions">
                                    <a href="bcso.php?edit=<?php echo $agent['id']; ?>" class="btn btn-edit">Modifier</a>
